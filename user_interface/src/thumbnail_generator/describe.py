...
class Describe():
  def gemini(image, style: str) -> str:
    description = model.generate_content([prompt.format(style), Image.fromarray(np.uint8(image)).convert('RGB')]).text
    return description

  def moondream(image, style: str, path="vikhyatk/moondream2", ft_path = None) -> str:
