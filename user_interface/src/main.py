...
def main(page: ft.Page):
    ...
    def call_frame_selection():
        ...
        best_frame_column.controls.append(
            ft.Text(f"Downloading video and extracting {frame_amount.value} frames...", size=15))
        best_frame_column.controls.append(ft.Text("Rating frames...", size=15))
        best_frame_column.update()

        best_frame_src = frame_selection(
            key_sentence.value, video_url.value, model_selection.value, frame_amount.value
        )
