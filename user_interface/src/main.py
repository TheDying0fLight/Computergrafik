...
def main(page: ft.Page):
    ...
    def pick_files_result(e: ft.FilePickerResultEvent):
        if e.files:
            ...
            selected_files.update()
        page.own_frame = selected_img.content.src
    ...
    model_frame_text = ft.Text(
        "You can enter the video URL and a key sentence below \n and choose a model which will select the best frame.", size=15)
    ...
    def call_frame_selection():
        ...
        print("BEST FRAME SRC:", best_frame_src)
    ...
    def frame_selection(key_sentence, video_url, model_selection, frame_amount):
        ...
        best_frame_column.controls.extend(
            [ft.Text(f"Downloading the video and extracting {frame_amount} frames...", size=15)])
        best_frame_column.controls.extend([ft.Text("Model rating the frames...", size=15)])
        best_frame_column.update()
    ...
    instruction_SD = ft.Text(
        "Insert Stable Diffusion styles, separated by commata, and choose the model to generate the prompt for Stable Diffusion.", size=20)
    ...
    submit_button_2 = ft.ElevatedButton(
        text="Submit", on_click=lambda e: call_thumbnail_generation(style.value, model_selection_2.value))
    ...
    def call_thumbnail_generation(style, model_selection):
        if page.own_frame is not None: page.best_frame = page.own_frame
        thumbnail_src = thumbnail_generation(style, model_selection, page.best_frame)
        thumbnail = ft.Image(src=thumbnail_src, width=16*50, height=9*50, fit=ft.ImageFit.CONTAIN)
        ...
        sd_out.controls.append(thumbnail_text)
        page.update()
        ...
        sd_out.controls.append(ft.ElevatedButton(
            "Save cartoonish thumbnail",
            icon=ft.Icons.SAVE,
            on_click=lambda _: save_file_dialog.save_file(),
            disabled=page.web,
        ))
        sd_out.controls.append(save_file_path)
        page.update()
        return

    def thumbnail_generation(style, model_selection, best_frame):
        sd_out.controls.clear()
        best_frame = Image.open(best_frame)
        sd_out.controls.append(ft.Text("Model describing the frame...", size=15))
        page.update()
        frame_prompt = Pipeline.describe_best_frame(best_frame, style, model_selection)
        ...
        with Image.fromarray(np.uint8(thumbnail)).convert('RGB') as f: f = f.save(thumbnail_src)
