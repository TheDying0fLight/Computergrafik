...
def get_prompts(transcript: str = None) -> dict[str,str]:
    ...
    pixel_values = None # TODO load image as pixels | load_image(image_path, max_num=12).to(torch.bfloat16).cuda()
    ...
    responses["positive"] = model.chat(tokenizer, pixel_values, '<image>\n' + prompt_positive, generation_config)
    responses["negative"] = model.chat(tokenizer, pixel_values, '<image>\n' + prompt_negative, generation_config)
